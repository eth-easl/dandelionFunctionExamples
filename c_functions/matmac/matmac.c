...
int mat_mac() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (set_size == 1) {
      IoBuffer *mat_buffer = dandelion_get_input(i, 0);
      size_t mat_size = mat_buffer->data_len;
      int64_t mat_rows = ((int64_t *)mat_buffer->data)[0];
      if (mat_size < 8 || mat_size % 8 != 0)
        return -2;
      mat_size = mat_size / 8 - 1;
      if (mat_rows > mat_size || mat_size % mat_rows != 0)
        return -3;
    }
  }

  if (c_set_size == 1) {
    IoBuffer *c_mat_buffer = dandelion_get_input(2, 0);
    ...
    if (a_set_size != 1 || b_set_size != 1)
      return -4;
    IoBuffer *a_mat_buffer = dandelion_get_input(0, 0);
    IoBuffer *b_mat_buffer = dandelion_get_input(1, 0);
