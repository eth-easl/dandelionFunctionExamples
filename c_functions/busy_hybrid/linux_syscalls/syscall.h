...
static inline int linux_connect(int socketfd, const struct sockaddr_in *addr,
                                int addrlen) {
  return __syscall(SYS_connect, socketfd, addr, addrlen);
}

static inline ptrdiff_t linux_send(int socketfd, const void *buf, size_t len,
                                   int flags) {
  return __syscall(SYS_sendto, socketfd, buf, len, flags, NULL, 0);
}

static inline ptrdiff_t linux_recv(int socketfd, void *buf, size_t len,
                                   int flags) {
  return __syscall(SYS_recvfrom, socketfd, buf, len, flags, NULL, NULL);
}

static inline int linux_close(int fd) { return __syscall(SYS_close, fd); }
